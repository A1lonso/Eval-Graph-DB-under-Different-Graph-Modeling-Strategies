...
class ResearchTransitiveOpt(Workload):
    ...
    def dataset_generator(self):
        ...
        for batch_num in range(0, total_people, batch_size):
            ...
            transitive_queries = [(
                """MATCH (p1:Person)
                WHERE p1.id >= $start AND p1.id < $end
    ...
    def benchmark__test__strong_collaboration_clusters(self):
        ...
        MATCH (a:Person)-[ab:COLLABORATED_WITH]-(b:Person),
            (b)-[bc:COLLABORATED_WITH]-(c:Person),
            (c)-[ca:COLLABORATED_WITH]-(a)
        WHERE a.id < b.id AND b.id < c.id  // Evita duplicatas
        RETURN a.name as person1, 
            b.name as person2, 
            c.name as person3,
            (ab.strength + bc.strength + ca.strength) as total_cluster_strength
