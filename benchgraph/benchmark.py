...
if __name__ == "__main__":
    ...
    for workload, queries in target_workloads:
        ...
        if generated_queries:
            ...
            #Added Yield check for generator dataset imports
            if hasattr(generated_queries, '__iter__') and not isinstance(generated_queries, (list, tuple)):
                # It's a generator - iterate through batches
                all_import_results = []
                for batch_num, batch_queries in enumerate(generated_queries):
                    log.info(f"Importing batch {batch_num + 1}...")
                    batch_result = client.execute(queries=batch_queries, num_workers=benchmark_context.num_workers_for_import)
                    all_import_results.extend(batch_result)
                ret = all_import_results
            else:
                # It's a regular list - execute directly
